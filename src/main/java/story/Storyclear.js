...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const clearSound = new Audio("music/Storyclear_sound.mp3");
    const endingBgm = new Audio("music/Storyclear_bgm2.mp3");
    
    endingBgm.volume = 0.5;
    endingBgm.loop = true;

    // 画面がロードされたら即座にクリアファンファーレを鳴らす
    clearSound.play().catch(err => console.log("ブラウザの制限：ユーザー操作後に音声が再生可能になります", err));
    ...
    setTimeout(() => {
        ...
        rollContainer.style.setProperty('--scroll-distance', `-${scrollDistance}px`);
